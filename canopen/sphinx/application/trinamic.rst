Trinamic Stpper Motor control with ros2_canopen
================================================
...
If you haven't already done so, follow the steps in the :ref:`Creating a Configuration Package`.
...
First :ref:`Setup CAN Controller` either using a virtual CAN interface or a real CAN interface.

The above launch file is configured to use the virtual CAN interface ``vcan0``. If you want to use a real CAN interface, you need to change the ``can_interface_name`` argument to ``can0`` in the launch file.
Also comment the fake slave launch when using real hardware by adding a ``#`` in front of the line ``ld.add_action(slave_node_1)``.
